"""
Single analysis entry point for GEMINI.

Processes all result folders (with gloss, without gloss, context, no context).
Computes TOP-1, TOP-10, avg Levenshtein (normalized, no-norm, diacritics-only).
Writes one Excel: experiment_results_analysis_gemini.xlsx

Sheets: TOP1_ACCURACY_NORM, TOP10_ACCURACY_NORM, AVG_LEVENSHTEIN_NORM,
        TOP1_ACCURACY_NO_NORM, ..., TOP1_ACCURACY_DIACRITICS_ONLY, ...
        (columns: GEMINI_with_gloss, GEMINI_without_gloss)
    ...
    for folder_path in folder_list:
        ...
        for excel_file in files:
            ...
            try:
                ...
                all_norm[key]["GEMINI"] = shared_calculate_metrics(df, "full")
                all_no_norm[key]["GEMINI"] = shared_calculate_metrics(df, "none")
                all_diac[key]["GEMINI"] = shared_calculate_metrics(df, "diacritics_only")
...
def create_excel_tables(
        ...
        for context in ["No", "Yes"]:
            for shot in ["0", "c1", "c2", "c3", "c4", "c5", "c6", "c7", "35"]:
                for script in ["Ar", "Rom"]:
                    ...
                    m_with = all_with.get(key, {}).get("GEMINI", {})
                    m_without = all_without.get(key, {}).get("GEMINI", {})
                    rows_top1.append({
                        "Context": context, "Shot": shot, "Script": script,
                        "GEMINI_with_gloss": round(m_with.get("top1_accuracy", 0), 2),
                        "GEMINI_without_gloss": round(m_without.get("top1_accuracy", 0), 2),
                    })
                    rows_top10.append({
                        "Context": context, "Shot": shot, "Script": script,
                        "GEMINI_with_gloss": round(m_with.get("top10_accuracy", 0), 2),
                        "GEMINI_without_gloss": round(m_without.get("top10_accuracy", 0), 2),
                    })
                    rows_avg_lev.append({
                        "Context": context, "Shot": shot, "Script": script,
                        "GEMINI_with_gloss": round(m_with.get("avg_levenshtein", 0), 2),
                        "GEMINI_without_gloss": round(m_without.get("avg_levenshtein", 0), 2),
                    })
        return {
    ...
    base_dir = os.path.dirname(os.path.abspath(__file__))
    output_file = os.path.join(base_dir, "experiment_results_analysis_gemini.xlsx")
...
if __name__ == "__main__":
    print("=" * 80)
    print("GEMINI: Single analysis (with gloss + without gloss)")
    print("=" * 80)
