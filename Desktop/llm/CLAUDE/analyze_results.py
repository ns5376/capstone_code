"""
Single analysis entry point for CLAUDE.
    ...
    for folder_path in folder_list:
        ...
        for excel_file in files:
            ...
            try:
                ...
                all_norm[key]["Sys 3"] = shared_calculate_metrics(df, "full")
                all_no_norm[key]["Sys 3"] = shared_calculate_metrics(df, "none")
                all_diac[key]["Sys 3"] = shared_calculate_metrics(df, "diacritics_only")
...
def create_excel_tables(
        ...
        for context in ["No", "Yes"]:
            for shot in ["0", "c1", "c2", "c3", "c4", "c5", "c6", "c7", "35"]:
                for script in ["Ar", "Rom"]:
                    ...
                    m_with = all_with.get(key, {}).get("Sys 3", {})
                    m_without = all_without.get(key, {}).get("Sys 3", {})
                    rows_top1.append({
                        "Context": context, "Shot": shot, "Script": script,
                        "Sys_3_with_gloss": round(m_with.get("top1_accuracy", 0), 2),
                        "Sys_3_without_gloss": round(m_without.get("top1_accuracy", 0), 2),
                    })
                    rows_top10.append({
                        "Context": context, "Shot": shot, "Script": script,
                        "Sys_3_with_gloss": round(m_with.get("top10_accuracy", 0), 2),
                        "Sys_3_without_gloss": round(m_without.get("top10_accuracy", 0), 2),
                    })
                    rows_avg_lev.append({
                        "Context": context, "Shot": shot, "Script": script,
                        "Sys_3_with_gloss": round(m_with.get("avg_levenshtein", 0), 2),
                        "Sys_3_without_gloss": round(m_without.get("avg_levenshtein", 0), 2),
                    })
        return {
    ...
    base_dir = os.path.dirname(os.path.abspath(__file__))
    output_file = os.path.join(base_dir, "experiment_results_analysis_claude.xlsx")
...
if __name__ == "__main__":
    print("=" * 80)
    print("CLAUDE: Single analysis (with gloss + without gloss)")
    print("=" * 80)
