"""
Run all OPENAI experiments for GPT-4o: no-context + context, with gloss then without gloss.
Set OPENAI_MODEL_NAME=gpt-4o before loading the experiment module. Full test set, sequential.
"""
import os
import sys

# Set model before any import that reads OPENAI_MODEL_NAME
...
def _patch_save_results_to_without_gloss():
    """Patch save_results to write to 'without gloss' folders."""
...
def main():
    print("=" * 80)
    print("OPENAI GPT-4o: Running WITH GLOSS (no-context + context)")
    print("=" * 80)
    exp.run_no_context_experiments(sample_size=SAMPLE)
    exp.run_with_context_experiments(sample_size=SAMPLE)

    print("\n" + "=" * 80)
    print("OPENAI GPT-4o: Running WITHOUT GLOSS (no-context + context)")
    print("=" * 80)
